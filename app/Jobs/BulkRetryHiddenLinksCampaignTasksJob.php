...
class BulkRetryHiddenLinksCampaignTasksJob implements ShouldQueue
{
    ...
    {
        $query = HiddenLinksCampaignTasks::query()->where('hidden_links_campaigns_id', $campaignId);
    ...
    private function resetAndDispatch(array $taskIds): int
    {
        ...
        foreach (array_chunk($taskIds, self::CHUNK_SIZE) as $chunk) {
            ...
            foreach ($tasks as $task) {
                PublishHiddenLinksJob::dispatch((int) $task->id, (int) ($task->dispatch_generation ?? 0))
