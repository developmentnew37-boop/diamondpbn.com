...
class LocalClientBillingReportService
{
    ...
    public function campaignsForClient(LocalClient $client): Collection
    {
        ...
        $this->appendCampaignRows($rows, ScheduleCampaign::class, 'sticky_campaign', $client, fn ($q) => $q->where('is_sticky_campaign', true));
    ...
    public function resolveFilters(Request $request): array
    {
        ...
        return [
            ...
            'label' => $this->buildFilterLabel($month, $fromDate, $toDate, $status, $campaignType),
        ];
    }
    ...
    private function buildFilterLabel(string $month, string $fromDate, string $toDate, string $status, string $campaignType): ?string
    {
        $parts = [];

        if ($month !== '') {
            $parts[] = Carbon::createFromFormat('Y-m', $month)->format('F Y');
        ...
        if ($status !== '') {
            $parts[] = $this->statusFilterOptions()[$status];
        }
    ...
    private function appendUnpaidModels(
        ...
        $query = $modelClass::query()
            ->select(self::PAYMENT_OPERATION_COLUMNS)
    ...
    private function appendCampaignRows(
        ...
        $query = $modelClass::query()
            ->select(self::BILLING_REPORT_COLUMNS)
            ->where('local_client_id', $client->id)
            ->whereNotNull('billing_total');
