(function () {
    const cfg = window.convertPostWizard;
    if (cfg) {
        initStep1(cfg);
    }
    ...
    function initStep1(config) {
        ...
        function renderCampaignOptions(items, append) {
            if (!append && (!items || items.length === 0)) {
                campaignList.innerHTML = '<div class="!px-4 !py-8 text-center text-gray-400 text-sm">No campaigns found. Try another search or create a live post campaign first.</div>';
                return;
            }
    ...
    function initScheduleStep(step) {
        ...
        buildBtn?.addEventListener('click', () => {
            ...
            pastHint.textContent = past > 0
                ? `${past} post(s) on dates before today will publish on ${step.today} (conversion day).`
