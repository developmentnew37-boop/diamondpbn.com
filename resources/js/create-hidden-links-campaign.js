...
import { initDynamicRadioGroup, initPopup, nestedPop, allowOnlyNumbers } from "./general";
    ...
    if (step__03) {
        ...
        function buildDomainRows(domains) {
            return domains.map((domain, index) => {
                ...
                return [
                    // ✅ Checkbox column (matches Blade)
                    `
            <input
                type="checkbox"
                name="bulk_sidebar_domain_select[]"
                class="sidebar_domains"
                id="sel-domain-check-box-${id}"
                value="${domain.id}"
            >
            `,
            ...
            `,
                    ...
                    // ✅ Domain name with label (clickable)
                    `
            <label
                for="sel-domain-check-box-${id}"
        ...
        window.__onEnterStep03 = async function () {
            ...
            try {
                ...
                const HRandomDt = window.HRandomDt;
                HRandomDt.clear();
                HRandomDt.rows.add(rows).draw(false);
