import { initPopup,initMultiCheckbox } from "../general";
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    createSet.addEventListener('click', async (e) => {
        ...
        try {
            let url = 'http://127.0.0.1:8000/api/admin/domain/set/initialize';
            ...
            if (res.status) {
                alert(`${res.message}`);
                window.location = `http://127.0.0.1:8000/admin/domains/set/create?id=${res.data.id}`;
                name = cat = '';
