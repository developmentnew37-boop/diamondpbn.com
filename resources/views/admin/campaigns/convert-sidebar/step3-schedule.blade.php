...
@section('title', 'Schedule sidebar dates')
...
@section('main-content')
    ...
    <div class="content-card w-full">
        @include('admin.campaigns.convert-sidebar.partials.stepper', ['currentStep' => 3])
        ...
        <div class="w-full max-w-full">
            <div class="convert-wizard-summary !mb-5" id="schedule_summary">
                Scheduling <strong>{{ $summary['convertible'] }}</strong> tasks — the date grid total must equal this count.
            </div>

            <form method="post" action="{{ route('admin.convert.sidebar.step4', $campaign->id) }}" id="schedule_form">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 !mb-5 sm:max-w-2xl">
                    <div>
                        ...
                        <input type="date" name="schedule_from_date" id="schedule_from_date" class="convert-wizard-input" required>
                    </div>
                    <div>
                        <label class="convert-wizard-field-label" for="schedule_to_date">To date</label>
                        <input type="date" name="schedule_to_date" id="schedule_to_date" class="convert-wizard-input" required>
                    </div>
                </div>
                ...
                <div class="convert-wizard-actions">
                    <a href="{{ route('admin.convert.sidebar.step2', $campaign->id) }}" class="convert-wizard-btn-secondary">
...
@push('scripts')
    ...
    <script src="{{ asset('js/convert-sidebar-campaign.js') }}?v={{ @filemtime(public_path('js/convert-sidebar-campaign.js')) ?: 1 }}"></script>
@endpush
