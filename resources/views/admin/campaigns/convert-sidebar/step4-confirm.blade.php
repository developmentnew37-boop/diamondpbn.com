...
@section('title', 'Confirm sidebar conversion')
...
@section('main-content')
    ...
    <div class="content-card w-full">
        @include('admin.campaigns.convert-sidebar.partials.stepper', ['currentStep' => 4])

        <p class="!mb-2">Source: <strong>{{ $campaign->campaign_no }}</strong></p>
        <p class="text-sm !mb-4">Mode: {{ $conversionMode }} · Tasks: {{ $summary['convertible'] }}</p>

        <div id="preflight_table" class="!mb-4 text-sm">Running domain pre-flight…</div>

        <form method="post" action="{{ route('admin.convert.sidebar.store') }}" id="convert_form">
            @csrf
            ...
            <label class="block text-sm !mb-2" for="campaign_no">New scheduled sidebar campaign #</label>
            ...
            <label class="flex items-start gap-2 text-sm !mb-4">
                ...
                <span>Domains with issues may fail individual tasks; I will use Retry on the converted campaign when sites recover.</span>
            </label>

            <div class="convert-wizard-actions">
                <a href="{{ route('admin.convert.sidebar.step3', ['campaign' => $campaign->id, 'mode' => $conversionMode]) }}"
...
@push('scripts')
    <script>
        (async function () {
            const url = @json(route('admin.convert.sidebar.preflight.campaign', $campaign->id));
            ...
            try {
                ...
                el.innerHTML = `<p class="!mb-2">Pre-flight (warn only): ${j.offline_domain_count ?? 0} domain(s) need attention.</p>
