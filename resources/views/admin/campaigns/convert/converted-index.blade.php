...
@section('title', 'Live → Dripfeed campaigns')

@push('style')
    @include('admin.campaigns.partials.campaign-list-table-styles')
    @include('admin.campaigns.convert.partials.converted-index-page-styles')
@endpush
        ...
        <div class="w-full flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="w-full md:flex-1 flex flex-col gap-2 min-w-0">
                ...
                <div class="breadcrumb flex-wrap">
                    ...
                    <div class="breadcrumb-item">
                        <a href="{{ route('admin.convert.post.converted.index') }}" class="breadcrumb-link">Dripfeed</a>
                        <span>›</span>
                    </div>
                    <div class="breadcrumb-item">Live → Dripfeed</div>
                </div>
            </div>
    ...
    <div class="w-full content-card">
        ...
        @if ($campaigns->total() > 0)
            <p class="text-sm text-gray-500 w-full !mb-3">
                ...
                · Live post campaigns moved to staggered dripfeed.
            </p>
        @else
            <p class="text-sm text-gray-500 w-full !mb-3">
                Live post campaigns moved to staggered dripfeed.
        ...
        <div class="w-full flex flex-col gap-2 !mb-2">
            ...
            <p class="text-sm text-gray-500">Select campaigns with checkboxes, then retry failed posts or remove local records.</p>
        </div>
            ...
            <table class="campaign-list-table display w-full min-w-[1200px] border border-gray-200 border-collapse text-sm whitespace-nowrap">
                ...
                <tbody>
                    @forelse ($campaigns as $index => $campaign)
                        @php
                            $total = (int) ($campaign->live_posts_total ?? $campaign->total_targets);
                            $completed = (int) ($campaign->live_posts_success ?? $campaign->completed_targets);
                            $failed = (int) ($campaign->live_posts_failed ?? $campaign->failed_targets);
                            $pending = max($total - ($completed + $failed), 0);
                            $progress = $total > 0 ? round(($completed / $total) * 100, 1) : 0;

                            $categoryLabel = optional($campaign->domainCategory)->name
                                ?? optional($campaign->sourceCampaign?->domainCategory)->name
                                ?? '-';
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border border-gray-200 font-sans !px-2 !py-3">
                                @if ($source)
                                    <a href="{{ route('admin.campaign.show', $source->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="15" class="text-center !py-4 text-gray-500 bg-gray-100 font-sans">
                                No converted schedule campaigns found.
                                <a href="{{ route('admin.convert.post.step1') }}" class="text-[var(--primary-color)] hover:underline">Convert a live post campaign</a>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
