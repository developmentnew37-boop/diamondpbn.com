...
@section('title', 'Convert post campaign')
...
@push('style')
    ...
    <style>
        ...
        #convert-step1-form [data-convert-campaign-picker] {
            overflow: visible;
        }

        #convert-step1-form [data-convert-campaign-picker]:focus-within {
            z-index: 40;
        }

        #convert-step1-form [data-convert-campaign-menu] {
            z-index: 50;
            max-width: 100%;
        }
        ...
        .convert-primary-btn:disabled {
            opacity: 0.5;
            cursor: not-allowed;
        }

        .convert-campaign-option.is-disabled {
            opacity: 0.55;
            cursor: not-allowed;
        }

        .convert-campaign-option-meta {
            display: block;
            font-size: 0.6875rem;
            color: #6b7280;
            margin-top: 0.15rem;
        ...
        .convert-step1-divider::before,
        .convert-step1-divider::after {
            content: '';
            flex: 1;
            height: 1px;
            background: #e5e7eb;
        ...
        .convert-report-url-result.is-success {
            background: #f0fdf4;
            border-color: #bbf7d0;
            color: #166534;
        }

        .convert-report-url-result.is-error {
            background: #fef2f2;
            border-color: #fecaca;
            color: #991b1b;
        }

        .convert-report-url-result.is-warning {
            background: #fffbeb;
            border-color: #fde68a;
            color: #92400e;
        }
    ...
    <div class="page-header w-full">
        <div class="flex flex-col gap-2">
            ...
            <div class="breadcrumb">
                ...
                <div class="breadcrumb-item">Convert campaign</div>
            </div>
        </div>
    </div>

    <div class="content-card w-full">
        @include('admin.campaigns.convert.partials.stepper', ['currentStep' => 1])

        <p class="text-sm text-gray-600 !mb-4">
            Convert an existing live PBN post campaign to dripfeed without creating new WordPress posts or reserving new articles.
        </p>

        <div id="convert-step1-form" class="w-full flex flex-col gap-3 max-w-3xl">
            <label for="selected_campaign_id"
            ...
            <div data-convert-campaign-picker class="w-full flex flex-col gap-3 p-2 relative" id="convert_campaign_picker">
                ...
                <button data-convert-campaign-btn type="button"
                    ...
                    <svg data-convert-campaign-chevron class="w-5 h-5 text-gray-400 transition-transform duration-200"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>

                <div data-convert-campaign-menu
                    ...
                    <div class="!px-3 !py-2 text-xs text-gray-500 border-b border-gray-100 bg-gray-50">
                        Recent campaigns (newest first). Type 2+ characters to search all.
                    </div>
                    <div data-convert-campaign-list class="max-h-64 overflow-y-auto"></div>
                    <div data-convert-campaign-loader class="hidden !px-4 !py-3 text-sm text-gray-500 border-t border-gray-100">
                        Loading…
                    </div>
            ...
            <p class="text-xs text-gray-500">Choose a live post campaign that is not already converted and has at least one published post.</p>

            <div class="convert-step1-divider">Or paste report URL</div>

            <label for="convert_report_url" class="convert-wizard-field-label">
                Campaign report URL
            </label>
            <div class="flex flex-col sm:flex-row gap-2">
                <input type="url" id="convert_report_url" class="convert-wizard-input flex-1"
                    placeholder="https://yoursite.com/campaign/report/campaign-no/token"
                    autocomplete="off">
            ...
            <p class="text-xs text-gray-500">
                Paste the customer report link you saved (PBN post campaigns only). The URL is matched locally — nothing is imported.
            </p>
...
@push('scripts')
    <script>
        window.convertPostWizard = {
            searchUrl: @json(route('admin.convert.post.search')),
            reportLookupUrl: @json(route('admin.convert.post.lookup-report-url')),
            csrfToken: @json(csrf_token()),
            step2Url: @json(url('/admin/convert/post/step/2')),
            preselectedId: @json($preselectedId),
            preselectedCampaignNo: @json($preselectedCampaign?->campaign_no),
        };
    </script>
    <script src="{{ asset('js/convert-post-campaign.js') }}?v={{ filemtime(public_path('js/convert-post-campaign.js')) }}"></script>
@endpush
