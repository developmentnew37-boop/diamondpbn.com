...
@section('main-content')
    ...
    <div class="w-full content-card !mt-3">
        ...
        <p class="text-sm text-gray-600 !mb-4">
            Each batch groups links with the <strong>same keyword and URL</strong>. Updating a batch updates that keyword/URL on <strong>all remote sites</strong>. Run: <code class="bg-gray-100 px-1 rounded">php artisan queue:work --queue=update_hidden_links</code>
        </p>
        ...
        @else
            <form action="{{ route('admin.hidden.link.campaign.update', $campaign->id) }}" method="POST" class="w-full">
                @csrf
                @method('PUT')
                        ...
                        <div class="border border-gray-200 rounded-lg !p-4 bg-gray-50">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <input type="text" name="batch_keyword[]" value="{{ old("batch_keyword.{$idx}", $batch['keyword']) }}"
                                ...
                                <div>
                                    ...
                                    <input type="url" name="batch_url[]" value="{{ old("batch_url.{$idx}", $batch['url']) }}"
                ...
                <div class="!mt-4 flex gap-2">
                    <button type="submit" class="!px-4 !py-2 bg-[var(--primary-color)] text-white rounded hover:opacity-90">Update batches &amp; sync to remote</button>
