...
@section('main-content')
    ...
    <div class="w-full content-card !p-4 !mb-4 flex flex-col gap-3">
        ...
        <p class="text-sm text-gray-600 max-w-3xl">
            Use <strong>+ Add row</strong> to add more keyword/URL pairs (e.g. one client URL across many sites, or multiple links in one post).
            Saving updates the database and queues a remote sync for published posts.
            <strong>At least one</strong> complete keyword + URL pair is required.
        </p>
        <form action="{{ route('admin.schedule.campaign.update.post.keywords', $campaignPost->id) }}" method="POST" id="schedule-single-keyword-form" class="w-full flex flex-col gap-3">
            @csrf
    ...
    <script type="importmap">
{
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var kwForm = document.getElementById('schedule-single-keyword-form');
            if (kwForm) {
