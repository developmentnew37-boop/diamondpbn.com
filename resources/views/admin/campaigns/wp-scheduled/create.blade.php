...
@section('title', 'Create WP Scheduled Campaign')
...
@section('main-content')
        ...
        <div class="w-full flex flex-wrap items-center">
            ...
            <div class="w-1/2 flex flex-wrap justify-end items-center">
                <a href="{{ route('admin.wp.schedule.campaign.index') }}"
                    class="flex !p-2 !py-3 text-[16px] font-normal w-fit justify-center duration:300 bg-green-500 whitespace-nowrap hover:bg-[var(--primary-color)] text-white rounded ">WP Scheduled
                    List</a>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.wp.schedule.campaign.store') }}" method="POST" id="campaign-form"
        class="w-full flex flex-wrap justify-between items-start content-card">
        @csrf
        <h2 class="text-xl capitalize !mb-4 bg-[var(--primary-color)] text-white w-fit !p-2 rounded">Create WP Scheduled Campaign
        </h2>
        ...
        <div class=" w-full !p-4 duration-500 transition-all opacity-0 translate-y-5  campaigns-section" id="campaign-info">
            <div class="w-full flex flex-col gap-5 bg-white  rounded">
                ...
                {{-- WP Scheduled: date distribution table (sum must equal post quantity) --}}
                <div class="w-full flex flex-col gap-3 !mt-4">
                    <label class="text-sm font-medium text-gray-700">Date distribution</label>
                    <p class="text-sm text-gray-600">Set how many posts per date. Click "Generate date table" then fill quantities. Total must equal Post Quantity above.</p>
                    <button type="button" id="wp-sch-generate-dates" class="!px-3 !py-2 rounded bg-blue-600 text-white text-sm w-fit hover:bg-blue-700">Generate date table</button>
                    <div id="wp-sch-date-distribution" class="!mt-2 max-w-2xl" style="display: none;">
                        <table class="w-full border border-gray-200 text-sm" id="wp-sch-date-table">
                            <thead>
                                <tr class="bg-gray-700 text-white">
                                    <th class="border !px-2 !py-2 text-left">Date</th>
                                    <th class="border !px-2 !py-2 text-left">Number of posts</th>
                                </tr>
                            </thead>
                            <tbody id="wp-sch-date-tbody"></tbody>
                        </table>
                        <p class="text-sm !mt-2">Sum: <strong id="wp-sch-date-sum">0</strong></p>
                    </div>
        ...
        <div class=" w-full !p-4 duration-500 transition-all opacity-0 translate-y-5   campaigns-section"
            ...
            <div class="w-full flex flex-wrap gap-5 bg-white  rounded">
                ...
                <div class="flex flex-col gap-4 w-[49%]">
                    <div class="w-full flex flex-col gap-3">
                        <label for="campaign-no" class="text-sm flex items-center ">Campaigns #
                            (Id)
                        </label>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var btn = document.getElementById('wp-sch-generate-dates');
            if (!btn || btn._wpSchBound) return;
            ...
            var distDiv = document.getElementById('wp-sch-date-distribution');
            var tbody = document.getElementById('wp-sch-date-tbody');
            var sumEl = document.getElementById('wp-sch-date-sum');
            ...
            function build() {
                ...
                if (qty < 1) { alert('Please enter Post quantity (at least 1).'); return; }
                ...
                while (cur <= to) {
                    ...
                    tr.innerHTML = '<td class="border !px-2 !py-2">'+dateStr+'</td><td class="border !px-2 !py-2"><input type="number" min="0" class="wp-sch-date-qty border border-gray-300 rounded !px-2 !py-1 w-24" data-date="'+dateStr+'" value="0"></td>';
                    tbody.appendChild(tr);
                    cur.setDate(cur.getDate()+1);
                }
                function up() {
                    var inputs = tbody.querySelectorAll('.wp-sch-date-qty');
                    var sum = 0, arr = [];
                    inputs.forEach(function(inp){ var v = parseInt(inp.value,10)||0; sum+=v; arr.push({date:inp.getAttribute('data-date'),quantity:v}); });
                    sumEl.textContent = sum;
                    dateQuantitiesInp.value = JSON.stringify(arr);
                }
                tbody.querySelectorAll('.wp-sch-date-qty').forEach(function(inp){ inp.addEventListener('input',up); inp.addEventListener('change',up); });
                up();
            }
            btn.addEventListener('click', build);
            btn._wpSchBound = true;

            // Allow same date: set To date min to From date so picker allows same day and later
            function syncToMin() {
            ...
            if (f) f.addEventListener('submit', function() {
                var rows = tbody.querySelectorAll('.wp-sch-date-qty');
                if (rows.length) {
                    var arr = [];
                    rows.forEach(function(inp){ arr.push({date:inp.getAttribute('data-date'),quantity:parseInt(inp.value,10)||0}); });
                    dateQuantitiesInp.value = JSON.stringify(arr);
                }
            });
        });
