...
class HiddenLinkCampaignDomainOrderTest extends TestCase
{
    ...
    private function createSchema(): void
    {
        ...
        Schema::create('hidden_links_campaigns', function (Blueprint $table) {
            ...
            $table->string('domain_method')->nullable();
        ...
        Schema::create('hidden_links_campaigns_tasks', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('hidden_links_campaigns_id');
            $table->unsignedBigInteger('hidden_links_campaigns_domain_id');
            $table->unsignedBigInteger('hidden_links_campaigns_link_id')->nullable();
            $table->string('status')->default('queued');
    ...
    public function test_show_and_report_list_domains_in_paste_order(): void
    {
        ...
        $campaign = HiddenLinksCampaign::query()->create([
            'campaign_no' => 'hidden-order-test',
            'admin_id' => $admin->id,
            'status' => 'completed',
            'total_targets' => count(self::PASTE_ORDER),
            'completed_targets' => count(self::PASTE_ORDER),
            'failed_targets' => 0,
            'report_token' => 'token-'.str_repeat('b', 48),
        ]);
        ...
        foreach (self::PASTE_ORDER as $index => $name) {
            ...
            $domainRow = HiddenLinksCampaignDomains::query()->create([
                'hidden_links_campaigns_id' => $campaign->id,
                'domain_id' => $domain->id,
            ]);

            $link = HiddenLinksCampaignLinks::query()->create([
                'hidden_links_campaigns_id' => $campaign->id,
            ...
            HiddenLinksCampaignTasks::query()->create([
                'hidden_links_campaigns_id' => $campaign->id,
                'hidden_links_campaigns_domain_id' => $domainRow->id,
                'hidden_links_campaigns_link_id' => $link->id,
                'status' => 'success',
                'attempt_count' => 0,
                'remote_id' => 'hid_'.$index,
            ]);
        }

        $show = $this->get(route('admin.hidden.link.campaign.show', $campaign->id));
        $show->assertOk();
        $showDomains = $show->viewData('campaignTasks')->pluck('domainRow.domain.name')->all();
        $this->assertSame(self::PASTE_ORDER, $showDomains);

        $report = $this->get(route('admin.hidden.link.campaign.report', [
