...
class PublishRemainingScheduleSidebarCampaignTasksTest extends TestCase
{
    ...
    public function test_publish_remaining_keeps_future_schedule_at_and_dispatches(): void
    {
        ...
        $successId = $this->insertTask($campaign->id, 'success', '2026-08-12 00:00:00');
        $futureA = $this->insertTask($campaign->id, 'queued', '2026-08-14 00:00:00');
        $futureB = $this->insertTask($campaign->id, 'queued', '2026-08-15 00:00:00');

        $summary = PublishRemainingScheduleSidebarCampaignTasksJob::publishRemaining($campaign->id, $this->counters);

        $this->assertSame(2, $summary['queued']);
        $this->assertFalse($summary['skipped']);

        $this->assertSame('2026-08-14 00:00:00', ScheduleSidebarCampaignTask::find($futureA)->schedule_at->format('Y-m-d H:i:s'));
        $this->assertSame('2026-08-15 00:00:00', ScheduleSidebarCampaignTask::find($futureB)->schedule_at->format('Y-m-d H:i:s'));
    ...
    public function test_publish_remaining_resets_failed_and_counters(): void
    {
        ...
        $this->insertTask($campaign->id, 'success', '2026-08-12 00:00:00');
        $failedId = $this->insertTask($campaign->id, 'failed', '2026-08-14 00:00:00', attemptCount: 5, lastError: 'down');
    ...
    public function test_publish_remaining_skips_paused_campaign(): void
    {
        ...
        $taskId = $this->insertTask($campaign->id, 'queued', '2026-08-14 00:00:00');
        ...
        $this->assertSame('queued', ScheduleSidebarCampaignTask::find($taskId)->status);
    ...
    private function makeCampaign(string $status, int $completed, int $failed, int $total): ScheduleSidebarCampaign
    {
        return ScheduleSidebarCampaign::query()->create([
            'campaign_no' => 'SSC-REM-'.Str::random(4),
